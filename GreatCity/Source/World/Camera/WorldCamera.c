...
GCWorldCamera *GCWorldCamera_Create(const float FoV, const float AspectRatio, const float Near, const float Far)
{
    ...
    WorldCamera->ProjectionMatrix =
        GCMatrix4x4_CreatePerspective(GCMathUtilities_DegreesToRadians(FoV), AspectRatio, Near, Far);
...
void GCWorldCamera_Pan(GCWorldCamera *const WorldCamera, const GCVector2 MouseDelta)
{
    ...
    const GCVector3 NegativeRightDirection =
        GCVector3_MultiplyByScalar(GCWorldCamera_GetRightDirection(WorldCamera), -1.0f);
...
void GCWorldCamera_Zoom(GCWorldCamera *const WorldCamera, const float MouseDelta)
{
    ...
    {
        WorldCamera->FocalPoint =
            GCVector3_Add(WorldCamera->FocalPoint, GCWorldCamera_GetForwardDirection(WorldCamera));
